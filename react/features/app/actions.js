...
export function redirectWithStoredParams(pathname: string) {
    return (dispatch: Dispatch<any>, getState: Function) => {
        ...
        const newLocationURL = new URL(locationURL.origin);
...
export function maybeRedirectToWelcomePage(options: Object = {}) {
    return (dispatch: Dispatch<any>, getState: Function) => {
        ...
        const localModerator = getState()['features/base/participants'].filter(p => p.local && p.role === PARTICIPANT_ROLE.MODERATOR);
        ...
        if (getState()['features/base/config'].enableWelcomePage) {
            setTimeout(
                () => {
                    localModerator.length ? dispatch(redirectWithStoredParams('/'))
                        : window.location.href = 'https://comrex.com';
                },
