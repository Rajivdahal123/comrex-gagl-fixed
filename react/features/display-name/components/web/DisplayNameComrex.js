 ...
 * @extends Component
 */
class DisplayNameComrex extends Component<Props, State> {
    ...
    render() {
        ...
        if (allowEditing && this.state.isEditing) {
            return (
                <input
                    className = 'editdisplaynamecomrex'
                    id = 'editdisplaynamecomrex'
 ...
 * }}
 */
function convertParticipantName(participant, codec, participantID, state, user) {
    if (participant.name === codec?.codec_ip) {
        return codec?.codec_name;
    }

    return getParticipantDisplayName(
        state, participantID) === user?.codec_ip ? user?.codec_name : getParticipantDisplayName(
        state, participantID);
}
...
function _mapStateToProps(state, ownProps) {
    ...
    const codec = state['features/base/settings'].codec;

    return {
        _configuredDisplayName: participant && participant.name,
        _nameToDisplay: convertParticipantName(participant, codec, participantID, state, user)
    };
}

export default translate(connect(_mapStateToProps)(DisplayNameComrex));
