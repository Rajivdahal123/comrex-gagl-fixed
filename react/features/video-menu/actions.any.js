 ...
 * @returns {Function}
 */
export function muteLocal(enable: boolean, mediaType: MEDIA_TYPE, localId, isEnsure) {
    return (dispatch: Dispatch<any>) => {
        const isAudio = mediaType === MEDIA_TYPE.AUDIO;

        if (localId && localId !== 'local') {
            APP.conference.commands.sendCommandOnce('update-mute-local', { value: JSON.stringify({
                id: localId,
                enable
            }) });
        }
        ...
        { isEnsure ? dispatch(isAudio ? setAudioMuted(enable, /* ensureTrack */ false)
 ...
 * @returns {Function}
 */
export function muteRemote(participantId: string, mediaType: MEDIA_TYPE, mute: boolean) {
    return (dispatch: Dispatch<any>) => {
        ...
        if (!mute) {
            ...
            APP.conference.commands.sendCommandOnce('unmute-remote-participant', {
                value: participantId
            });
        }
    };
