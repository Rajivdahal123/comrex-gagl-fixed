...
class InviteContactsForm extends AbstractAddPeopleDialog<Props, State> {
    ...
    state = {
        ...
        loading: false,
        loadingFirst: true,
        inviteItems: JSON.parse(jitsiLocalStorage.getItem('inviteItems')) || []
    };
    ...
    componentDidMount() {
        ...
        // const { isCodec, count, isAddNumber } = this.props;
        //
        // if (isCodec && !count && !isAddNumber) {
        //     this.props.dispatch({
        //         type: 'UPDATE_PROPS'
        //     });
        //
    ...
    render() {
        ...
        const getInvitedList = JSON.parse(jitsiLocalStorage.getItem('inviteItems')) || [];
        const invitedListItem = [];

        getInvitedList.map((item, key) => {
            invitedListItem.push(key < getInvitedList.length - 1 ? `${item.value},` : `${item.value}`);
        });
        ...
        // this.state.loadingFirst && !isAddNumber && this.state.loading ? <div style = {{ overflow: 'hidden' }}>
        //
        //     <div
        //         style = {{ display: 'flex',
        //             flexDirection: 'row' }}>
        ...
        //         <div
        //             style = {{ marginLeft: 3,
        //                 color: '#b8c8e0' }}>Initializing Call To  {invitedListItem}
        //             <div
        ...
        //             defaultValue = { this.state.inviteItems.map(item => {
        //                 return {
        //                     ...item,
    ...
    _onSelectionChange(selectedItems) {
        ...
        if (!this.props.isAddNumber) {
            jitsiLocalStorage.setItem('inviteItems',
                JSON.stringify(selectedItems.map(({ elemBefore, tag, ...item }) => item)));
        }
    }
    ...
    _onSubmit() {
        ...
        jitsiLocalStorage.setItem('inviteItems',
            JSON.stringify(inviteItems.map(({ elemBefore, tag, ...item }) => item)));
        if (this.props.isAddNumber) {
     ...
     */
    _renderFormActions() {
        const { inviteItems, loading } = this.state;
        const { t, isAddNumber } = this.props;
        ...
        return (
            <div className = { `invite-more-dialog invite-buttons${this._isAddDisabled() ? ' disabled' : ''}` }>
                <div className = { 'invite-buttons-codec' }>
                    {this.props.isAddNumber ? <a
                    ...
                    <a
                        ...
                        <div
                            className = { 'spinner' } >
                            {loading ? <Spinner /> : isAddNumber ? t('addPeople.addToList') : t('addPeople.add')}
                        </div>
                    </a>
                </div>
            </div>
    ...
    _renderErrorMessage() {
        ...
        console.log('supportString', supportString);
        console.log('supportLink', supportLink);
        if (!supportLink) {
            return null;
        }
...
function _mapStateToProps(state) {
    ...
    const count = state['features/base/dialog'].componentProps?.count;
