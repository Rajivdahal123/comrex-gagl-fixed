...
class Filmstrip extends Component<Props> {
    ...
    render() {
        ...
        return (
            <div
                className = { 'filmstrip filmstrip-container filmstrip-container-500' }
                        ...
                        <div
                            ...
                            {_participants.map((p, index) => (
                                <div
                                    ...
                                    style = {{
                                        display: p.email === this.props.settings.emailModerator && this.props.settings.mode === 'silent' ? 'none' : 'block',
                                        paddingBottom: 22
                                    }}>
                                    <Thumbnail
                                        key = {
                                            p.local ? 'local' : `remote_${p.id}`
                                        }
                                        participantID = { p.id } />
                                </div>
                            ))}
...
function _mapStateToProps(state) {
    ...
    const videosClassName = `filmstrip__videos${visible}`;
