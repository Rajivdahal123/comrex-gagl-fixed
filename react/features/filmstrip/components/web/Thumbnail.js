...
import { IconAccount, IconAudio, IconHangup } from '../../../base/icons';
...
import {
    getLocalParticipant,
    getParticipantById,
    getParticipantCount, PARTICIPANT_ROLE,
    pinParticipant
...
import { ControllerMeeting } from '../../../controller-meeting';
import { DisplayName } from '../../../display-name';
import DisplayNameComrex
    from '../../../display-name/components/web/DisplayNameComrex';
...
import RequestUnmuteButton from './RequestUnmuteButton';
...
class Thumbnail extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.handleSendRequestUnmute = this.handleSendRequestUnmute.bind(this);
    }
    ...
    componentDidUpdate(prevProps: Props, prevState: State) {
        if (prevProps._audioTrack !== this.props._audioTrack) {
            // console.log('===============+> track compare debug', prevProps._audioTrack, this.props._audioTrack);
    ...
    _listenForAudioUpdates() {
        ...
        // console.log('listen for audio update ===========>', _audioTrack);

        if (_audioTrack) {
            const { jitsiTrack } = _audioTrack;

            // console.log('jitisi track ============>', jitsiTrack);
            // if (jitsiTrack && !jitsiTrack.storedMSID) {
            //     this._updateAudioLevel(0.1);
            // }
    ...
    _stopListeningForAudioUpdates(audioTrack) {
        // console.log('stop listening audio======>');
        if (audioTrack) {
    ...
    _updateAudioLevel(audioLevel) {
        // console.log('jitsi audio updated =====>', audioLevel);
        this.setState({
            audioLevel
        });
    }
     ...
     */
    handleSendRequestUnmute() {
        APP.conference.commands.sendCommandOnce('request-unmute', {
    ...
    _renderParticipant() {
        ...
        const { id, local, loadableAvatarUrl, isJigasi, avatarURL } = _participant;
        const { audioLevel, canPlayEventReceived, volume } = this.state;
        const styles = this._getStyles();
        const containerClassName = this._getContainerClassName();
        ...
        return (
            <div>
                ...
                <div
                    id = 'item-recording'
                    className = { _participant.raisedHand ? 'item-recording-active item-recording-500' : 'item-recording item-recording-500' }>
                    <div
                        style = {{ display: 'flex',
                            flexDirection: 'row',
                            ...
                            alignItems: 'center' }}>
                        <div >
                            { avatarURL && !_participant?.isJigasi ? <div className = 'wrap-avatar-toolbox'>
                                <Gravatar
                                    email = { avatarURL }
                                    size = { 40 }
                                    default = 'mp' />
                                ...
                                : <ToolbarButton
                                    ...
                                    'toolbar.accessibilityLabel.account'
                                    ) }
                        ...
                        <div
                            className = { 'display-name-comrex display-user-name-800' } >
                            <DisplayNameComrex
                                allowEditing = { _participant.local }
                                elementID = { `participant_${id}_name` }
                                participantID = { id } />
                        </div>
                        <div className = { 'audio-wave-box audio-wave-box-hidden' }>
                            <AudioWave
                                value = { audioLevel }
                                isJigasi = { _participant.isJigasi } />
                        </div>
                        <div
                            className = { 'flex-audio' }
                            style = {{ display: 'flex' }} >
                            <div style = {{ marginLeft: 10 }}>
                                <MuteAudioButton
                                    key = 'mute'
                                    isJigasi = { isJigasi }
                                    isLocal = { _participant.local }
                                    participantID = { id } />
                            </div>
                            {_isModerator === true && !isJigasi ? <div style = {{ marginLeft: 10 }}>
                                <KickOutButton
                                    isJigasi = { isJigasi }
                                    isLocal = { _participant.local }
                                    participantID = { id } />
                            </div> : '' }
                            {_isModerator === true && !isJigasi && !_participant.local ? <div style = {{ marginLeft: 10 }}>
                                <RequestUnmuteButton onClick = { this.handleSendRequestUnmute } />
                            </div> : '' }
                        </div>

                    </div>

                    <div className = { 'audio-wave-box audio-wave-box-500 hidden-above-500' }>
                        <AudioWave
                            value = { audioLevel }
                            isJigasi = { _participant.isJigasi } />
                    </div>
...
function _mapStateToProps(state, ownProps): Object {
    ...
    const { localFlipX, user } = state['features/base/settings'];
    ...
    return {
        ...
        _isModerator: getLocalParticipant(state).role === PARTICIPANT_ROLE.MODERATOR,
...
export default translate(connect(_mapStateToProps)(Thumbnail));
