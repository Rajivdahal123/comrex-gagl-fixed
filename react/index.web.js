...
import { kickParticipant, PARTICIPANT_ROLE } from './features/base/participants';
...
window.addEventListener('beforeunload', () => {
    ...
    const codecParticipant = participants.filter(con => con.isJigasi);

    const localParticipant = participants?.find(item => item.local);

    if (conference.room && localParticipant && localParticipant.role === PARTICIPANT_ROLE.MODERATOR) {
        participants.forEach(p => {
            if (!p.local && p.id) {
                APP.store.dispatch(kickParticipant(p.id));
            }
        });
        APP.store.dispatch(kickParticipant(codecParticipant[0].id));
    }
});
