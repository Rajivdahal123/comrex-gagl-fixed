...
import {
    NOTIFICATION_TIMEOUT, SHOW_NOTIFICATION,
    showNotification
...
const commands = {
    ...
    REQUEST_UNMUTE: 'request-unmute',
    UPDATE_MUTE_LOCAL: 'update-mute-local',
    UNMUTE_REMOTE_PARTICIPANT: 'unmute-remote-participant'
};

function sendCommandOnce(command, value) {
    if (!room) {
        return;
    }
...
export default {
     ...
     */
    createInitialLocalTracksAndConnect(roomName, options = {}, syncDevice) {

        let tryCreateLocalTracks = [];
        let errors;

        if (syncDevice) {
            const value = this.createInitialLocalTracks(options);

            tryCreateLocalTracks = value.tryCreateLocalTracks;
            errors = value.errors;
        }
     ...
     * @returns {Promise}
     */
    async init({ roomName, syncDevice }) {
        ...
        if (isPrejoinPageEnabled(APP.store.getState())) {
            ...
            if (syncDevice) {
                ...
                tracks = await tryCreateLocalTracks;
        ...
        const [ tracks, con ] = await this.createInitialLocalTracksAndConnect(roomName, initialOptions, syncDevice);
    ...
    _setupListeners() {
        ...
        room.on(JitsiConferenceEvents.USER_STATUS_CHANGED, (id, status) => {
            ...
            const user = room.getParticipantById(id);
            // console.log('update user status ====>', user, id, status);
            if (user) {
        ...
        room.on(JitsiConferenceEvents.TRACK_AUDIO_LEVEL_CHANGED, (id, lvl) => {
            ...
            if (this.isLocalId(id)
                && this.localAudio && this.localAudio.isMuted()) {
                    // console.log('id audo local check ====>', id, this.localAudio);
                newLvl = 0;
            }
            ...
            // console.log('new audio level =>', id, newLvl);
            APP.UI.setAudioLevel(id, newLvl);
        });

        room.on(JitsiConferenceEvents.TRACK_MUTE_CHANGED, (track, participantThatMutedUs) => {
            // console.log('muted us =====>', track, participantThatMutedUs);
        ...
        room.addCommandListener(this.commands.defaults.REQUEST_UNMUTE,
            ({ value }) => {
                const localParticipant = APP.store.getState()['features/base/participants']?.find(item => item.local);

                if (localParticipant?.id === value) {
                    APP.store.dispatch(showNotification({
                        titleKey: 'notify.requestUnmute',
                        descriptionKey: 'notify.requestDescribe'
                    }));
                }
            }
        );
        room.addCommandListener(this.commands.defaults.UPDATE_MUTE_LOCAL, ({ value }) => {
        ...
        room.addCommandListener(this.commands.defaults.UNMUTE_REMOTE_PARTICIPANT, ({ value }) => {
            const participants = APP.store.getState()['features/base/participants'];
            const localParticipant = participants.find(p => p.local);

            if (localParticipant?.id === value) {
                APP.store.dispatch(muteLocal(false, 'audio'));
            }
        });
    ...
    _onDeviceListChanged(devices) {
        ...
        const promises = [];
        const audioWasMuted = this.isLocalAudioMuted();
        // console.log('=======> audio muted check in device list change');
        const videoWasMuted = this.isLocalVideoMuted();
        ...
        promises.push(
            mediaDeviceHelper.createLocalTracksAfterDeviceListChanged(
                ...
                .then(tracks => {
                    ...
                    // console.log('mute sync promice list =========>', muteSyncPromises)
